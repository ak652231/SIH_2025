...
import { Mountain, Camera, Compass, Plus, DollarSign, Clock, Users, LogOut } from "lucide-react"
...
const DashboardPage = () => {
  ...
  const validateMerchantForm = () => {
    const newErrors = {}
    if (!merchantFormData.productName) newErrors.productName = "Product name is required"
    if (!merchantFormData.productPhoto) newErrors.productPhoto = "Product photo is required"
    if (!merchantFormData.description) newErrors.description = "Description is required"
  ...
  const validateTourGuideForm = () => {
    const newErrors = {}
    if (!tourGuideFormData.tourTitle) newErrors.tourTitle = "Tour title is required"
    if (!tourGuideFormData.locationsAreas) newErrors.locationsAreas = "Locations/Areas is required"
    if (!tourGuideFormData.meetingPoint) newErrors.meetingPoint = "Meeting point is required"
    if (!tourGuideFormData.duration) newErrors.duration = "Duration is required"
    if (!tourGuideFormData.tourCategory) newErrors.tourCategory = "Tour category is required"
    if (!tourGuideFormData.tourDescription) newErrors.tourDescription = "Tour description is required"
    if (!tourGuideFormData.pricePerPerson) newErrors.pricePerPerson = "Price per person is required"
    if (!tourGuideFormData.maxGroupSize) newErrors.maxGroupSize = "Maximum group size is required"
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-teal-50 to-emerald-100 relative overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto px-4 py-8">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold text-emerald-800">Welcome, {userData.name}!</h1>
            <p className="text-emerald-600 capitalize">
              {userData.userType === "merchant" ? "Merchant" : "Tour Guide"} Dashboard
            </p>
          </div>
          <button
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="mb-6">
            ...
            >
              ...
              {userData.userType === "merchant" ? "Add New Product" : "Add New Tour"}
            </button>
          </div>
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl p-6 border border-emerald-100 mb-6">
              ...
              {userData.userType === "merchant" ? (
                ...
                <form onSubmit={handleMerchantSubmit} className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-colors ${
                        errors.productName ? "border-red-500" : "border-emerald-200"
                      }`}
                  ...
                  <div>
                    <CloudinaryUpload
                      onUploadSuccess={(url) => setMerchantFormData({ ...merchantFormData, productPhoto: url })}
                      label="Product Photo"
    ...
    src={
      typeof merchantFormData.productPhoto === "string"
        ? merchantFormData.productPhoto
        : URL.createObjectURL(merchantFormData.productPhoto) // if it's a File object
    }
  ...
  />
                ...
                <p className="text-xs text-gray-500 text-center mt-2">PNG, JPG, or PDF (max. 5MB)</p>
                    ...
                    {errors.productPhoto && <p className="mt-1 text-sm text-red-600">{errors.productPhoto}</p>}
                  ...
                  <div>
                    <label className="block text-sm font-medium text-emerald-700 mb-1">Cost (₹)</label>
                ...
                <form onSubmit={handleTourGuideSubmit} className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-emerald-700 mb-1">Tour Title</label>
                      <input
                        type="text"
                        name="tourTitle"
                        ...
                        className={`w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-colors ${
                          errors.tourTitle ? "border-red-500" : "border-emerald-200"
                        }`}
                        placeholder="Short name of the tour"
                      />
                      {errors.tourTitle && <p className="mt-1 text-sm text-red-600">{errors.tourTitle}</p>}
                    </div>

                    <div>
                      <label className="block text-sm font-medium text-emerald-700 mb-1">Duration</label>
                      <div className="relative">
                        ...
                        <input
                          ...
                          onChange={handleTourGuideInputChange}
                          className={`w-full pl-10 pr-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-colors ${
                            errors.duration ? "border-red-500" : "border-emerald-200"
                          }`}
                  ...
                  <div>
                    <label className="block text-sm font-medium text-emerald-700 mb-1">Locations/Areas Covered</label>
                    <input
                      type="text"
                      name="locationsAreas"
                      ...
                      onChange={handleTourGuideInputChange}
                      className={`w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-colors ${
                        errors.locationsAreas ? "border-red-500" : "border-emerald-200"
                      }`}
                      placeholder="City, village, landmark, etc. (comma separated)"
                    />
                    {errors.locationsAreas && <p className="mt-1 text-sm text-red-600">{errors.locationsAreas}</p>}
                  </div>

                  <div>
                    <label className="block text-sm font-medium text-emerald-700 mb-1">Meeting Point</label>
                    <input
                      type="text"
                      name="meetingPoint"
                      ...
                      onChange={handleTourGuideInputChange}
                      className={`w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-colors ${
                        errors.meetingPoint ? "border-red-500" : "border-emerald-200"
                      }`}
                      placeholder="Specific place customers should come to"
                    />
                    {errors.meetingPoint && <p className="mt-1 text-sm text-red-600">{errors.meetingPoint}</p>}
                  </div>

                  <div>
                    <label className="block text-sm font-medium text-emerald-700 mb-1">Tour Category</label>
                    <select
                      name="tourCategory"
                      value={tourGuideFormData.tourCategory}
                      onChange={handleTourGuideInputChange}
                      className={`w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-colors ${
                        errors.tourCategory ? "border-red-500" : "border-emerald-200"
                      }`}
                    >
                    ...
                    {errors.tourCategory && <p className="mt-1 text-sm text-red-600">{errors.tourCategory}</p>}
                  </div>

                  <div>
                    <label className="block text-sm font-medium text-emerald-700 mb-1">Tour Description</label>
                    <textarea
                      ...
                      rows={3}
                      className={`w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-colors resize-none ${
                        errors.tourDescription ? "border-red-500" : "border-emerald-200"
                      }`}
                      placeholder="Short paragraph about the experience"
                    />
                    {errors.tourDescription && <p className="mt-1 text-sm text-red-600">{errors.tourDescription}</p>}
                  </div>

                  <div>
                    <label className="block text-sm font-medium text-emerald-700 mb-1">Highlights</label>
                    <textarea
                      name="highlights"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-emerald-700 mb-1">Price per Person (₹)</label>
                      <div className="relative">
                        <DollarSign className="absolute left-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-emerald-400" />
                        <input
                          type="number"
                          ...
                          onChange={handleTourGuideInputChange}
                          className={`w-full pl-10 pr-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-colors ${
                            errors.pricePerPerson ? "border-red-500" : "border-emerald-200"
                          }`}
                          placeholder="Price"
                        />
                      </div>
                    ...
                    <div>
                      <label className="block text-sm font-medium text-emerald-700 mb-1">Min Group Size</label>
                      <div className="relative">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-emerald-700 mb-1">Max Group Size</label>
                      <div className="relative">
                        <Users className="absolute left-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-emerald-400" />
                        <input
                          type="number"
                          ...
                          onChange={handleTourGuideInputChange}
                          className={`w-full pl-10 pr-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-colors ${
                            errors.maxGroupSize ? "border-red-500" : "border-emerald-200"
                          }`}
                          placeholder="Max"
                        />
                      </div>
                  ...
                  <div>
                    <CloudinaryUpload
                      onUploadSuccess={(url) => setTourGuideFormData({ ...tourGuideFormData, tourPhotos: url })}
                      label="Tour Photos"
                      className={`w-full border-2 border-dashed ${
    ...
    src={
      typeof tourGuideFormData.tourPhotos === "string"
        ? tourGuideFormData.tourPhotos
        : URL.createObjectURL(tourGuideFormData.tourPhotos) // if it's a File object
    }
    alt="Uploaded Product"
    className="w-full h-40 object-cover rounded-lg border border-emerald-200"
  />
                ...
                <p className="text-xs text-gray-500 text-center mt-2">PNG, JPG, or PDF (max. 5MB)</p>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-emerald-700 mb-1">What's Included</label>
                      <textarea
                        name="whatsIncluded"
                    ...
                    <div>
                      <label className="block text-sm font-medium text-emerald-700 mb-1">What's Not Included</label>
                      <textarea
                  ...
                  <div>
                    <label className="block text-sm font-medium text-emerald-700 mb-1">Cancellation Policy</label>
                    <select
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl p-6 border border-emerald-100">
            <h3 className="text-xl font-bold text-emerald-800 mb-4">Your Profile</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
              ...
              {userData.userType === "merchant" && (
                <>
                  <div>
                    ...
                    <p className="font-medium text-emerald-800">{userData.businessName}</p>
                  </div>
                  <div>
                    <p className="text-sm text-emerald-600">Business Type</p>
                    <p className="font-medium text-emerald-800 capitalize">{userData.businessType}</p>
                  </div>
                </>
              )}
              {userData.userType === "tourguide" && (
                <div>
                  ...
                  <p className="font-medium text-emerald-800">{userData.languagesSpoken}</p>
                </div>
              )}
            </div>
          </div>
