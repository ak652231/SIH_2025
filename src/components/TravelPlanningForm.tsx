...
export default function TravelPlanningForm({ onSubmit, isLoading = false }: TravelPlanningFormProps) {
  ...
  const saveToRecentSearches = (location) => {
    const updatedSearches = [location, ...recentLocationSearches.filter((item) => item.id !== location.id)].slice(0, 5)
  ...
  const formatDisplayName = (item) => {
    ...
    if (item.address) {
      ...
      if (item.address.state_district && item.address.state_district !== parts[0]) {
    ...
    switch (station.id) {
      ...
      case "travel":
        return (
          <div className="space-y-6">
            <div>
              <Label className="text-lg font-medium">How will you travel?</Label>
              <Select value={formData.transport_mode} onValueChange={(value) => updateField("transport_mode", value)}>
            ...
            <div>
              <Label className="text-lg font-medium">What's your travel pace?</Label>
              <Select value={formData.pace} onValueChange={(value) => updateField("pace", value)}>
                <SelectTrigger className="mt-2 text-lg p-4">
            ...
            <div>
              <Label htmlFor="base_location_name" className="text-lg font-medium">
                Starting location
              </Label>
              <div className="relative mt-2" ref={locationSuggestionRef}>
                ...
                {showLocationSuggestions && (
                  <div className="absolute z-10 mt-1 w-full bg-white shadow-lg rounded-md border border-gray-200 max-h-60 overflow-auto">
                    ...
                    {!isLocationLoading && locationSuggestions.length === 0 && recentLocationSearches.length > 0 && (
                      <>
                        ...
                        {recentLocationSearches.map((search) => (
                          <div
                            ...
                            onClick={() => handleLocationSuggestionClick(search)}
                          >
      ...
      case "preferences":
        return (
          <div className="space-y-6">
            <div>
              <Label className="text-lg font-medium mb-4 block">What interests you?</Label>
              <div className="grid grid-cols-2 gap-4">
                {["nature", "culture", "temple", "adventure", "food", "history"].map((interest) => (
            ...
            <div>
              <Label className="text-lg font-medium mb-4 block">Must-visit places</Label>
              <div className="grid grid-cols-1 gap-4">
                {["betla_np", "bodh_gaya", "ranchi", "jamshedpur", "deoghar"].map((place) => (
                  <div key={place} className="flex items-center space-x-2">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => {
                        if (checked) {
                          updateField("must_visit", [...formData.must_visit, place])
                        } else {
                          updateField(
                            "must_visit",
          ...
          <div className="space-y-6">
            <div className="flex items-center space-x-2">
              <Checkbox
                ...
                onCheckedChange={(checked) => updateField("family_trip", checked)}
              />
            ...
            <div className="flex items-center space-x-2">
              <Checkbox
                ...
                onCheckedChange={(checked) => updateField("accessibility_needs", checked)}
              />
    ...
    <div className="max-w-5xl mx-auto p-6">
          ...
          {STATIONS.map((station, index) => {
            ...
            return (
              <div key={station.id} className="relative z-10 flex flex-col items-center">
                <div className={`relative w-24 h-28 transition-all duration-300 ${isActive ? "scale-110" : ""}`}>
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center text-black font-bold">
                      <div className="text-[10px] leading-tight text-center px-1 mt-2">
                        {station.name.split(" ").map((word, i) => (
                          <div key={i} className="uppercase tracking-tight font-extrabold">
                            {word}
                          </div>
                        ))}
                      </div>
                  ...
                  <div
                    className={`absolute bottom-18 left-1/2 -translate-x-1/2 w-20 h-10 rounded-t-full border-2 transition-colors duration-300 shadow-lg ${isActive ? "bg-gradient-to-b from-yellow-300 to-yellow-500 border-yellow-600" : isCompleted ? "bg-gradient-to-b from-green-300 to-green-500 border-green-600" : "bg-gradient-to-b from-yellow-300 to-yellow-500 border-yellow-600"}`}
                  >
                ...
                {isActive && (
                  <div
                    className={`absolute top-1/2 text-4xl transition-all duration-600 ease-in-out z-20 ${isAnimating ? `transform ${currentStation < STATIONS.length - 1 ? "translate-x-32" : "translate-x-0"} scale-110` : "scale-100"}`}
                    style={{
                      left: "calc(100% + 1px)",
      ...
      <div className="bg-white rounded-lg border-2 border-red-600 p-8 min-h-[400px]">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h2 className="text-2xl font-bold text-gray-900">{STATIONS[currentStation].name}</h2>
        </div>
