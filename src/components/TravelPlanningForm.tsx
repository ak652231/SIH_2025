...
import { Users, Clock, Car, Heart, Loader2 } from "lucide-react"
  ...
  const handleSubmit = () => {
    ...
    const processedData = {
      ...
      base_location: formData.base_location
        ? formData.base_location.split(",").map((coord) => Number.parseFloat(coord.trim()))
        : [23.36, 85.33],
      ...
      case "travel":
        return (
          <div className="space-y-6">
            ...
            <div>
              <Label htmlFor="base_location" className="text-lg font-medium">
                Starting location (latitude, longitude)
              </Label>
              <Input
                id="base_location"
                placeholder="e.g., 23.36, 85.33"
            ...
            return (
              <div key={station.id} className="relative z-10 flex flex-col items-center">
                ...
                >
                  ...
                  <div
                    className={`
                      absolute bottom-18 left-1/2 -translate-x-1/2 w-20 h-10 rounded-t-full border-2 transition-colors duration-300 shadow-lg
                      ${
                        isActive
                ...
                {isActive && (
                  <div
                    className={`
                      absolute top-1/2 text-4xl transition-all duration-600 ease-in-out z-20
                      ${isAnimating ? `transform ${currentStation < STATIONS.length - 1 ? "translate-x-32" : "translate-x-0"} scale-110` : "scale-100"}
                    `}
                    style={{
                      left: "calc(100% + 1px)",
