...
const ServiceCards: React.FC = () => {
  ...
  const features = [
    {
      icon: <Bot className="h-8 w-8 text-green-1" />,
      title: "AI Itinerary & Multilingual Chatbot",
      description:
        "Personalized trip plans and 24x7 assistance in multiple languages.",
    },
    {
      icon: <BadgeCheck className="h-8 w-8 text-green-1" />,
      title: "Verified Providers via Blockchain",
      description:
        "Secure bookings, guide verification, and digital certifications.",
    },
    {
      icon: <Map className="h-8 w-8 text-green-1" />,
      title: "Interactive Maps & AR/VR Previews",
      description:
        "Preview sites like Netarhat, Patratu, Betla, Hundru, and Deoghar.",
    },
    {
      icon: <TrainFront className="h-8 w-8 text-green-1" />,
      title: "Real-Time Transport & Geo-Info",
      description: "Live location, routes, and local travel updates.",
    },
    {
      icon: <ShoppingBag className="h-8 w-8 text-green-1" />,
      title: "Local Marketplace",
      description:
        "Discover handicrafts, homestays, cultural events, and eco-tours.",
    },
    {
      icon: <Users className="h-8 w-8 text-green-1" />,
  ...
  return (
    <div
      className={`relative w-full ${montserrat.variable} ${poppins.variable}`}
    >
      <h3 className="text-center font-poppins text-2xl font-bold text-green-1 mb-16 tracking-tight">
        <span className="text-black relative inline-block after:content-[''] after:absolute after:w-12 after:h-1 after:bg-green-1 after:bottom-[-8px] after:left-1/2 after:transform after:-translate-x-1/2">
          Key Features
        </span>
      </h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-6xl mx-auto mt-16">
        {features.map((feature, index) => (
          <div
            key={index}
            className="feature-card bg-white rounded-lg shadow-lg p-6 transform transition-all hover:scale-105 z-20 border-t-4 border-green-1"
          >
            <div className="flex items-start gap-4">
              ...
              <div>
                <h3 className="text-green-1 font-poppins font-bold text-lg mb-2 tracking-tight">
                  {feature.title}
                </h3>
                <p className="text-gray-700 font-montserrat leading-relaxed text-[15px]">
                  {feature.description}
                </p>
              </div>
            </div>
